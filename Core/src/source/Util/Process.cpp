...
Process::Process(const PROCESS_INFORMATION processInformation) :
	...
	m_Executable {L""},
	m_CommandLine {L""},
	m_WorkingDirectory {L""},
	m_Suspended {false},
	m_Attached {false}
{
	memcpy(&m_ProcessInformation, &processInformation, sizeof(PROCESS_INFORMATION));
}
...
Process::Process() :
	...
	m_Attached {false}
{
	m_ProcessInformation.hProcess = GetCurrentProcess();
	m_ProcessInformation.hThread = GetCurrentThread();
	m_ProcessInformation.dwProcessId = GetProcessId(m_ProcessInformation.hProcess);
	m_ProcessInformation.dwThreadId = GetThreadId(m_ProcessInformation.hThread);
}
